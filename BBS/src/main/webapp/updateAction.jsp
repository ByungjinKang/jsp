...
<%
    ...
    try {
        ...
        String query = "SELECT Title, Content, Board_ID, User_ID FROM Post WHERE POST_code = ?";
        ...
        if (rs.next()) {
            ...
            String postUserID = rs.getString("User_ID");
            ...
            rs = pstmt.executeQuery();
            if (rs.next()) {
                adminValue = rs.getString("Admin");
                if (adminValue.equals("1")) {
                    isAdmin = true;
                }
            }

            if (userID.equals(postUserID) || isAdmin) {
                ...
                PrintWriter script = response.getWriter();
                script.println("<script>");
                script.println("alert('글을 수정할 수 있는 권한이 없습니다.')");
                script.println("location.href = 'view.jsp?postID=" + postID + "'");
                script.println("</script>");
            }
        } else {
