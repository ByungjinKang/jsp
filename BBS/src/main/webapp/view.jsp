...
<%
    ...
    if (userID != null) {
        ...
        try {
            ...
            if (!historyCheckRs.next()) { // 방문 기록이 없는 경우에만 기록 저장
                ...
                // 조회수 업데이트
                String updateViewCountSql = "UPDATE Post SET ViewCount = ViewCount + 1 WHERE POST_code = ?";
    ...
    try {
        ...
        if (postRs.next()) {
            ...
            // 게시물이 속한 게시판의 ID 가져오기
            String boardID = null;
            String boardIDSql = "SELECT Board_ID FROM Board WHERE Board_ID IN (SELECT Board_ID FROM Post WHERE POST_code = ?)";
            PreparedStatement boardIDPstmt = conn.prepareStatement(boardIDSql);
            boardIDPstmt.setInt(1, postID);
            ResultSet boardIDRs = boardIDPstmt.executeQuery();
            if (boardIDRs.next()) {
                boardID = boardIDRs.getString("Board_ID");
            }
    %>
    ...
    <div class="container">
        ...
        %>
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <% } %>
                    <% if (userID != null && (userID.equals(commentWriter) || isAdmin)) { %>
        ...
        <% if (userID != null && (userID.equals(writer) || isAdmin)) { %>
        <a href="update.jsp?postID=<%= postID %>&boardID=<%= boardID %>" class="btn btn-primary">수정</a>
        <a href="deleteAction.jsp?postID=<%= postID %>&boardID=<%= boardID %>" class="btn btn-primary">삭제</a>
        <% } %>
    </div>
